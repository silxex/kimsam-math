  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:8}}>
      ...
      <div style={{flex:1,height:12,background:"#F0F0F0",borderRadius:99,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${Math.min(xp/maxXp*100,100)}%`, background:"linear-gradient(90deg,#FDCB6E,#FF9F43)", transition:"width 0.5s"}}/>
      </div>
      <span style={{fontSize:13,fontWeight:800,color:"#FF9F43"}}>{xp}/{maxXp}</span>
    </div>
  );
}
  ...
  return (
    <div style={{padding:"0 4px"}}>
      <div style={{textAlign:"center",marginBottom:28}}>
        ...
        <h1 style={{margin:0,fontSize:26,fontWeight:900,color:"#2D3436"}}>2학년 1학기 수학</h1>
        <p style={{margin:"6px 0 0",color:"#B2BEC3",fontSize:13}}>단원을 선택해서 공부해보세요!</p>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {UNITS.map((u)=>(
          <button key={u.id} onClick={()=>onSelect(u.id)}
            style={{ background:u.light, border:`2.5px solid ${u.border}`, borderRadius:20, padding:"16px 12px", cursor:"pointer", textAlign:"left" }}>
            <div style={{fontSize:28,marginBottom:6}}>{u.emoji}</div>
            <div style={{fontSize:13,fontWeight:900}}>{u.id}단원</div>
            <div style={{fontSize:11,fontWeight:700,color:u.color}}>{u.title}</div>
            <div style={{marginTop:8}}>
               {Array.from({length:3},(_,j)=>(<span key={j} style={{opacity:j<(stars[u.id]||0)?1:0.2}}>⭐</span>))}
            </div>
          </button>
        ))}
      </div>
...
function QuizScreen({ unitId, onBack, onStar }) {
  ...
  const next = () => {
    if (qIdx+1>=5) {
      const s = score+(status==="correct"?1:0);
      onStar(unitId, s>=5?3:s>=3?2:1);
      setDone(true);
    } else {
  ...
  if (done) return (
    <div style={{textAlign:"center",padding:"20px 0"}}>
      <div style={{fontSize:72,marginBottom:12}}>{score>=4?"🏆":"🥈"}</div>
      <div style={{fontSize:24,fontWeight:900,color:unit.color}}>{unit.title} 완료!</div>
      <button onClick={onBack} style={{marginTop:20, padding:"12px 24px", background:unit.color, color:"white", border:"none", borderRadius:12}}>목록으로</button>
    </div>
  );
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        ...
        <div style={{flex:1, height:10, background:"#F0F0F0", borderRadius:99, overflow:"hidden"}}>
          <div style={{height:"100%", width:`${progress}%`, background:unit.color, transition:"width 0.4s"}}/>
        </div>
      </div>
      <div style={{background:unit.light, border:`2px solid ${unit.border}`, borderRadius:24, padding:"22px 18px", marginBottom:16, textAlign:"center"}}>
        <div style={{fontSize:17, fontWeight:800}}>{q.q}</div>
      </div>
      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10}}>
        {q.choices.map((c,i)=>(
          <button key={i} onClick={()=>pick(c)} style={{
            padding:"14px", borderRadius:16, background:selected===c?(status==="correct"?"#E8F5E9":"#FFEBEE"):"white",
            border:selected===c?(status==="correct"?"2px solid #66BB6A":"2px solid #EF9A9A"):"2px solid #E0E0E0",
            fontWeight:800, cursor:status?"default":"pointer"
          }}>{c}</button>
        ))}
      </div>
      {status && (
        <button onClick={next} style={{width:"100%", marginTop:20, padding:"14px", background:unit.color, color:"white", border:"none", borderRadius:16, fontWeight:900}}>
...
export default function App() {
  ...
  return (
    <div style={{
      ...
      minHeight: "100vh", width: "100vw", background: "#EEF2FF", padding: "20px", boxSizing: "border-box"
    }}>
      <style>{`
      ...
      <div style={{
        ...
        width: "100%", maxWidth: "400px", boxShadow: "0 20px 60px rgba(0,0,0,0.08)"
      }}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:20,paddingBottom:16,borderBottom:"2px dashed #F0F0F0"}}>
           <div style={{fontSize:13,fontWeight:800,color:"#B2BEC3"}}>Kim Saem Math</div>
           <div style={{background:"#FFF9E6",borderRadius:99,padding:"4px 14px",fontSize:13,fontWeight:900,color:"#FF9F43"}}>⭐ {totalStars}개</div>
        </div>
        {screen==="home" ? (
          <UnitSelect stars={stars} onSelect={uid=>{ setActiveUnit(uid); setScreen("unit"); }}/>
        ) : (
          <QuizScreen unitId={activeUnit} onBack={()=>setScreen("home")} onStar={(id,s)=>setStars(p=>({...p,[id]:Math.max(p[id]||0,s)}))}/>
        )}
      </div>
    </div>
  );
